"use strict";

const OBTbuttonELement = "[data-testid=button]";

module.exports = {
  "@tags": ["stage", "sanity", "portal", "travel arranger"],

  before: function(driver) {
    driver.globals.waitForConditionTimeout = 50000;
    driver.windowMaximize();
  },

  "step 1 - login to portal": driver => {
    const login = driver.page.login();
    login.fillLoginDetails(driver.globals.users.portalUser9);
  },

  "step 2 - Validate home header items": driver => {
    // Travel arranger name
    driver.waitForTextByCss(
      "[data-testid=hero-name-of-traveler]:nth-child(2)",
      "Portal Sanity"
    );
    // Travel arranger name and book your travelers... text
    driver.waitForTextByCss(
      "[data-testid=hero-welcome-message]",
      "Hello Portal Sanity , book your travelers' next trips"
    );
    // Book my full trip button
    driver.waitForTextByCss(OBTbuttonELement, "CONTINUE");
    driver.waitForAttributeContainsByCss(
      OBTbuttonELement,
      "href",
      "https://accounts.mycwt.com/idp/startSSO.ping?PartnerSpId=KDS&ACSIdx=0"
    );
  },

  "step 3 - Validate travel arranger my travelers list": driver => {
    // click on my travelers link
    driver.waitAndClickByCss("[data-testid=header-navigation-myTravelers]");
    // check title
    driver.waitForTextByCss("[data-testid=page-title]", "My travelers view");
    // check subtitles
    driver.waitForTextByCss(
      "[data-testid=arranger-policies]",
      "- You can view and modify your travelers profile by accessing ‘My Travel Profile’"
    );
    // search pre-text
    // driver.waitForAttributeContainsByCss('[data-testid=my-travelers-search-input]', 'placeholder', 'Search Traveler'); //TODO: have a defect
    // search icon
    driver.waitForAttributeContainsByCss(
      "[data-testid=input-icon-search]",
      "class",
      "CwtIcons__StyledIcon-sc-1zecbe-0 styles__StyledIcon-n24pwb-0 lcarwX CwtIcons-sc-1zecbe-1 bThmWo"
    );
  },

  "step 4 - Validate second traveler": driver => {
    // traveler name
    driver.waitForTextByCss(
      "[data-testid=traveler-2] [data-testid=ta-name]",
      "Portal Sanity Sanity One"
    );
    // upcoming trip
    driver.waitForTextByCss(
      "[data-testid=traveler-2] [data-testid=upcoming-trip-link-2]",
      "Trip to Tokyo"
    );
    // date
    driver.waitForTextByCss(
      "[data-testid=traveler-2] [data-testid=ta-trip-dates]",
      "Jun 4 - Jun 10, 2025"
    );
    // missing hotel
    // driver.waitForTextByCss('[data-testid=traveler-1] [data-testid=ta-ma-indicator]', 'Missing Hotel'); //TODO: have defect
    // View all trips link
    driver.waitForTextByCss(
      "[data-testid=view-all-trips-link-2]",
      "view all trips"
    );
    // Book hotel button
    driver.waitForTextByCss("[data-testid=book-hotel-btn-2]", "BOOK A HOTEL");
  },

  "step 5 - Validate second traveler (no trips)": driver => {
    // traveler name
    // driver.waitForTextByCss('[data-testid=traveler-2] [data-testid=ta-name]', 'Portal Sanity Sanity Four');  // TODO: have a defect. #614
    // no upcoming trips text
    driver.waitForTextByCss(
      "[data-testid=traveler-3] [data-testid=ta-details]",
      "no upcoming trips"
    );
    // View all trips link
    // driver.waitForTextByCss('[data-testid=view-all-trips-link-3]', 'view all trips');
    // Book hotel button
    driver.waitForTextByCss("[data-testid=book-hotel-btn-3]", "BOOK A HOTEL");
  },

  "step 6 - Validate travel arranger search": driver => {
    // add existing text in the search field
    driver.waitAndSetValueByCss(
      "[data-testid=my-travelers-search-input]",
      "sanity one"
    );
    // validate traveler exist in the list
    driver.waitForTextByCss(
      "[data-testid=traveler-1] [data-testid=ta-name]",
      "Portal Sanity Sanity One"
    );
    // click on clear text button
    driver.waitAndClickByCss("[data-testid=input-icon-search]");
    // add non-existing text in the search field
    driver.waitAndSetValueByCss(
      "[data-testid=my-travelers-search-input]",
      "bla bla"
    );
    // validate search image
    // driver.waitForAttributeContainsByXpath('//*[@id="shell"]/div[1]/div/div/div[2]/img', 'src', 'data:image/svg+xml;base64,');
    // validate not found text
    driver.waitForTextByCss(
      "[data-testid=conditional-state-title]",
      "we couldn’t find any matches. \nmake sure your search is spelled correctly."
    );
    // click on clear text button
    driver.waitAndClickByCss("[data-testid=input-icon-search]");
  },

  "step 7 - Validate travel arranger my travelers links": driver => {
    // // click on second traveler trip      //TODO: we have a defect here.
    driver.waitAndClickByCss("[data-testid=upcoming-trip-link-2]");
    // traveler trip details - breadcrumb links
    driver.waitForTextByCss("[data-testid=home-link]", "HOME");
    driver.waitForTextByCss("[data-testid=my-travelers-link]", "MY TRAVELERS");
    driver.waitForTextByCss(
      "[data-testid=traveler-trips]",
      "PORTAL SANITY SANITY ONE"
    );
    // trip title
    driver.waitForTextByCss(
      "[data-testid=hero-title]",
      "Portal Sanity Sanity One's Trip to Tokyo"
    );
    driver.waitForTextByCss(
      "[data-testid=hero-dates]",
      "Jun 4 - Jun 10, 2025 | 7 days"
    );
    // Home
    driver.waitAndClickByCss("[data-testid=myCWT-logo]");

    // back to my travelers list
    driver.waitAndClickByCss("[data-testid=header-navigation-myTravelers]");

    // click on second traveler "view all trips" link
    driver.waitAndClickByCss(
      "[data-testid=view-all-trips-link-2]",
      "view all trips"
    );
    // trip title (traveler trips page)
    driver.waitForTextByCss(
      "[data-testid=trip-971131] [data-testid=trip-name]",
      "Trip to\nTokyo"
    );
    driver.waitForTextByCss(
      "[data-testid=trip-971131] [data-testid=trip-dates]",
      "Jun 4 - Jun 10, 2025 | 7 days"
    );
    // back to my travelers list
    driver.waitAndClickByCss("[data-testid=header-navigation-myTravelers]");
    // // click on second traveler "book a hotel" link   //TODO: have a defect
    // driver.waitAndClickByCss('[data-testid=book-hotel-btn-1]');
    // driver.waitForUrlToContain('http://new-ci.mycwt.com/book-a-hotel', 20000);
  },

  "step 8 - Logout": driver => {
    driver.waitAndClickByCss("[data-testid=account-dropdown-button]");
    driver.pause(1000);
    driver.waitAndClickByCss("[data-testid=header-logout]");
  }
};

//TODO: Add new traveler with no booking capability (sub with no booking)
