...
module.exports = {
  ...
  '@tags': ['production', 'sanity', 'portal', 'home', 'header'],
  ...
  'step 2 - Validate home header items': (driver) => {
    // driver.waitForTextByCss('[data-testid=hero-name-of-traveler]', 'Portal Sanity'); //todo: have a defect (en_US)
    ...
    driver.waitForAttributeContainsByCss(OBTbuttonELement, 'href', 'https://accounts.mycwt.com/idp/startSSO.ping?PartnerSpId=portal%3Aprod%3Acwt%3Asaml2');
  },
  ...
  // 'step 4 - Validate travel alerts': (driver) => { //TODO: cannot test alerts in production, the data is real and keep changing.
  //   // Home title
  ...
  'step 5 - OBT link': (driver) => {
    ...
    // hover text
    driver.waitForTextByCss('span[class^=BookingButton]', 'Powered by "Concur"');
    // click on OBT link
    driver.waitAndClickByCss('[data-testid=pill-button]');
    // Switch to the second tab [1]
    //driver.switchToTab(1);
    // Validate second tab URL
    // driver.waitForUrlToContain('www.concursolutions.com/home.asp', 20000);
    // Close second tab
    //driver.closeWindow();
    //driver.pause(1000);
